    ...
    private async Task CreateUpdatePaymentIntentAsync(ShoppingCart cart, long total)
{
    ...
    else
    {
        ...
        if (existingIntent.Status == "succeeded")
        {
            ...
            var newIntent = await service.CreateAsync(newOptions);
            cart.PaymentIntentId = newIntent.Id;
            cart.ClientSecret = newIntent.ClientSecret;
        }
        else
        {
            ...
            await service.UpdateAsync(cart.PaymentIntentId, updateOptions);
        }
    }
}
